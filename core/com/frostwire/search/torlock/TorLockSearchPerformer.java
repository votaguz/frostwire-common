...
public class TorLockSearchPerformer extends TorrentRegexSearchPerformer<TorLockSearchResult> {
    ...
    private static final String HTML_REGEX = "(?is).*?<a href=\\\"/tor/(?<torrentid>.*?).torrent\\\"><img.*?<td><b>Name:</b></td><td>(?<filename>.*?).torrent</td></tr>.*?<b>Size:</b></td><td>(?<filesize>.*?) in .*? file.*?</td>.*?<td><b>Added:</b></td><td>Uploaded on (?<time>.*?) by .*?color:#FF5400.*?>(?<seeds>\\d*?)</b>.*? seeders.*?";
    ...
    /**
     public static void main(String[] args) throws Exception {
         ...
         final SearchMatcher detailMatcher = SearchMatcher.from(detailPattern.matcher(new MaxIterCharSequence(resultHTML,4*resultHTML.length())));
